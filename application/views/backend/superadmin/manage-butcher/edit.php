...
<?php 
    $butcher = $this->db->where('id', $param1)->get('app_butcher')->row_array();
?>

<form method="POST" class="d-block ajaxForm" enctype="multipart/form-data" action="<?php echo route('manage_butcher/update_butcher/'.$butcher['id']); ?>">
    <div class="form-row">
        ...
        <?php 
            echo $butcher['photo'] 
                ? '<a target="_blank" href="' . base_url('uploads/butcher_photo/' . $butcher['photo']) . '">View</a>' 
                : '';  
        ?>    
        </label>
...
$(".ajaxForm").submit(function(e) {
    var form = $(this);
    ajaxSubmit(e, form, showAllbutcher);
});
