...
class Superadmin extends CI_Controller {
  ...
  public function manage_vyapari($param1 = '', $param2 = '', $param3 = ''){
    ...
    if($param1 == 'print2')
    {
       // set_time_limit(300);
        ...
        $this->load->view('backend/superadmin/vyapari/printid2', $page_info);
    }
  ...
  //-------------------------- new 2024 ---------------------------------------//
    ...
    public function cattle_prebooking($param1 = '', $param2 = '', $param3 = '')
    {
        if ($param1 == 'list')
        {
          $this->load->view('backend/superadmin/cattle_prebooking/list');
        }
        
        if ($param1 == 'list-boxes')
        {
          $pageDate['dataReport'] = $this->deonar_model->get_all_cattle_pre_reg_reports($this->input->post());
          $this->load->view('backend/superadmin/cattle_prebooking/list-boxes', $pageDate);
        }        
        
        if($param1 == 'ajaxlist')
        {
            $post = $this->input->post();
            $data = $this->deonar_model->get_all_cattle_pre_reg($post);
            echo json_encode($data);
        }
        ...
        {
          $post = $this->input->post();
          $response = $this->deonar_model->reg_cattle_pre_booking($post);
          echo $response;
        }
        ...
        {
          $post = $this->input->post();
          $response = $this->deonar_model->reg_cattle_pre_booking2($post);
          echo $response;
        }
        ...
        {
          $post = $this->input->post();
          $response = $this->deonar_model->uploadPrebookingCSV($post);
          echo $response;
        }        
        
        if($param1 == 'delete'){
          $response = $this->deonar_model->cattle_pre_booking_delete($param2);
          echo $response;
        }
    
        if(empty($param1)){
          $page_data['folder_name'] = 'cattle_prebooking';
          $page_data['page_title'] = 'Cattle Prebooking';
          $this->load->view('backend/index', $page_data);
        }
    } 
...
//-------------------------- new 2024 ---------------------------------------//
    ...
    public function reports($param1 = '', $param2 = '', $param3 = '')
    {
        ...
        if($param1 == 'dawanwala')
        {
            $page_data['page_name']  = 'vyapari/reports/dawanwala';
        ...
        if($param1 == 'hourly-outward')
        {
            ...
            // Build the query
            $this->db->select("DATE_FORMAT(exit_date, '%Y-%m-%d %H:00:00') AS exit_date_hour, COUNT(vyapari_id) AS Number_of_buffalo");
            $this->db->from('app_qrcode');
            $this->db->where('status', 'exit');
            $this->db->where('DATE(exit_date)', $selected_date);
            $this->db->group_by("DATE_FORMAT(exit_date, '%Y-%m-%d %H:00:00')");
            $query = $this->db->get();
            
            // Get the result
            $hourly = $query->result();
            
            $page_data['hourly']  = $hourly;
            
            $page_data['page_name']  = 'vyapari/reports/hours_report';
            $page_data['page_title'] = 'Hourly Outward';
            $this->load->view('backend/index', $page_data);
        }
        
        if($param1 == 'daily-report')
        {
            $page_data['page_name']  = 'vyapari/reports/daily_report';
            $page_data['page_title'] = 'Daily Report';
            $this->load->view('backend/index', $page_data);
        }
  ...
  public function aadhar_vyapari_detail(){
    ...
    public function certificate_to_tag_update(){
        $app_qrcode = $this->db->get('app_qrcode')->result();
        foreach($app_qrcode as $row):
            $tag = get_type_by_id('cattle_pre_booking', 'certificate_no', $row->qrcode, 'tag_no');
            $qrcode = $row->qrcode;
